...
mservice1 = "https://mservice1-0e6fe7cb6ed771e7f.7d67968.vpc-lattice-svcs.ap-southeast-1.on.aws"
mservice1sec = "https://mservice1-0e6fe7cb6ed771e7f.7d67968.vpc-lattice-svcs.ap-southeast-1.on.aws/secure"

avaurl = 'https://public-keys.prod.verified-access.ap-southeast-1.amazonaws.com/'
...
def signer(endpoint):
  ...
  signer = SigV4Auth(session.get_credentials(), 'vpc-lattice-svcs', 'ap-southeast-1')
  endpoint = endpoint
  data = "null"
  headers = {}
...
def depacker(portal):
  ...
  try:
    if "X-Amzn-Ava-User-Context" in portal:
      ...
      # Fix padding for base64 decoding if needed
      jwt_headers += '=' * (-len(jwt_headers) % 4)
      decoded_jwt_headers = base64.b64decode(jwt_headers, altchars='-_')
      ...
      try:
        ...
        pub_key = req.text
         ...
         # Dummy key to allow flow to continue (verification will fail but safely)
